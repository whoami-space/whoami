...
<script lang="js" >
    ...
    let sum = select.reduce((items, current) => items + current, 0)

    currentValue = sum

    return counter.update((value) => value + currentValue);
...
{#if data.stepFour }
    ...
    <div class="flex flex-col gap-medium">
        <h1 class="text-maximal text-center" >Step 4</h1>
    ...
    <button on:click={data.goTo("/explore/tests/mbti/result", data.result)} on:click={Counter([selectedOne, selectedTwo])} >
